...
//
// ASCOM Rotator driver for photonWanderer
//
...
namespace ASCOM.photonWanderer.Rotator
{
    ...
    //
    // Your driver's DeviceID is ASCOM.photonWanderer.Rotator
    //
    // The COM Guid attribute sets the CLSID for ASCOM.photonWanderer.Rotator
    // The COM ClassInterface/None attribute prevents an empty interface called _photonWanderer from being created and used as the [default] interface
    //
    ...
    /// </summary>
    [ComVisible(true)]
    [Guid("7175e701-671b-4b4c-adc3-12095c3ec87d")]
    [ProgId("ASCOM.photonWanderer.Rotator")]
    [ServedClassName("ASCOM Rotator Driver for photonWanderer")] // Driver description that appears in the Chooser, customise as required
    ...
    public class Rotator : ReferenceCountedObjectBase, IRotatorV4, IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="photonWanderer"/> class. Must be public to successfully register for COM.
        /// </summary>
        public Rotator()
        {
            try
            {
                ...
                tl = new TraceLogger("", "photonWanderer.Driver"); // Remove the leading ASCOM. from the ProgId because this will be added back by TraceLogger.
                SetTraceState();
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.photonWanderer.Rotator", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        // PUBLIC COM INTERFACE IRotatorV4 IMPLEMENTATION
            ...
            try
            {
                ...
                _ = Task.Run(() =>
                {
                    try
                    {
                        ...
                        RotatorHardware.SetConnected(uniqueId, true);
                        connectedState = true;
            ...
            set
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        RotatorHardware.SetConnected(uniqueId, false);
